...
def orders_silver():
    orders_df = process_order()
    return orders_df

@dp.table
@dp.expect_or_drop("valid_quantity", "quantity <= 0")
def orders_quarentine():
    orders_df = process_order()
    orders_df = orders_df.withColumn("quarentine_timestamp", F.current_timestamp())
    return orders_df
...
@dp.expect_all(rules)
def books_raw():
    schema = "book_id STRING, tittle STRING, author STRING, price DOUBLE, updated TIMESTAMP"
